...
/**
 * 聚范中汽车礼物的动画 所有偏移量，位移量根据opt计算或者屏幕宽度计算，或者资源尺寸百分比计算，以求达到不同分辨率下可以适配的效果
 * 
 * @author tpx
 * 
 */
public class JufanCarSurfaceView extends BaseAnimSurfaceView {
	...
	public JufanCarSurfaceView(Context context, AttributeSet attrs) {
		super(context, attrs);
	}
	 ...
	 * 
	 */
	public JufanCarSurfaceView(Context context) {
		super(context);
	}
	...
	private void initView() {
		...
		String driverStr = BASE_PATH + "/live_car.png";
		String carLightStr = BASE_PATH + "/live_car_light.png";
		String carInFireStr = BASE_PATH + "/car_in_fire/car_in_fire";
		String frontWheelFireStr =  BASE_PATH + "/front_wheel_fire/front_wheel_fire";
		String behindWheelFireStr = BASE_PATH + "/behind_wheel_fire/behind_wheel_fire";
		String leftWheelSmogStr = BASE_PATH + "/left_wheel_smog/left_wheel_smog";
		String rightWheelSmogStr =BASE_PATH +  "/right_wheel_smog/right_wheel_smog";
		String carDotStr = BASE_PATH + "/live_car_dot.png";
		String carOutFireStr = BASE_PATH + "/car_out_fire/car_out_fire";
		...
		if (mCarBmp == null || mCarLightBmp == null) {
			if (mOnDriverListener != null) {
				mOnDriverListener.onError("汽车动画");
			}
			state = STATE_FINISH;
		}
		// 加载车后火的序列帧
		...
		// 加载车上小亮点图片
		mCarDotBmp = AnimBitmapLoader.getInstance().getBitmap(getContext(),
				carDotStr, opt);
		if (mCarDotBmp == null) {
			if (mOnDriverListener != null) {
				mOnDriverListener.onError("汽车动画");
	...
	private void drawCar(Canvas canvas) {
		canvas.save();
		canvas.translate(mCarMoveStartX + mCarMoveOffsetXDistance
				+ mCarDriftedOffsetXDistance, mCarMoveStartY
				+ mCarMoveOffsetYDistance + mCarDriftedOffsetYDistance);
	...
	private void drawCarLeftLight(Canvas canvas) {
		canvas.save();
		canvas.translate(mCarLightMoveStartX + mCarLightOffsetX
				+ mCarLeftLightAddOffsetX * mCarLightScale
				+ mCarDriftedOffsetXDistance, mCarLightMoveStartY
				+ mCarLightOffsetY + mCarLeftLightAddOffsetY * mCarLightScale
				+ mCarDriftedOffsetYDistance);
	...
	private void drawCarRightLight(Canvas canvas) {
		canvas.save();
		canvas.translate(mCarLightMoveStartX + mCarLightOffsetX
				+ mCarRightLightAddOffsetX * mCarLightScale
				+ mCarDriftedOffsetXDistance, mCarLightMoveStartY
				+ mCarLightOffsetY + +mCarRightLightAddOffsetY * mCarLightScale
				+ mCarDriftedOffsetYDistance);
		mPaint.setAlpha(mCarLightAlpha);
		canvas.scale(mCarLightScale, mCarLightScale);
		canvas.drawBitmap(mCarLightBmp, 0, 0, mPaint);
	...
	private void drawFrontWheelFire(Canvas canvas) {
		...
		canvas.save();
		canvas.translate(mFrontWheelFireX + mFrontWheelFireXOffset,
				mFrontWheelFireY + mFrontWheelFireYOffset);
		canvas.drawBitmap(mFrontWheelFireFrames[mFrontWheelFireIndex], 0, 0,
				null);
		canvas.restore();
	}
	...
	private void drawBehindWheelFire(Canvas canvas) {
		...
		canvas.save();
		canvas.translate(mBehindWheelFireX + mBehindWheelFireXOffset,
				mBehindWheelFireY + mBehindWheelFireYOffset);
		canvas.drawBitmap(mBehindWheelFireFrames[mBehindWheelFireIndex], 0, 0,
				null);
		canvas.restore();
	}
	...
	private void drawLeftWheelSmog(Canvas canvas) {
		...
		canvas.save();
		canvas.translate(mLeftWheelSmogX + mLeftWheelSmogXOffset,
	...
	private void drawRightWheelSmog(Canvas canvas) {
		...
		canvas.save();
		canvas.translate(mRightWheelSmogX + mRightWheelSmogXOffset,
				mRightWheelSmogY + mRightWheelSmogYOffset);
		canvas.drawBitmap(mRightWheelSmogFrames[mRightWheelSmogIndex], 0, 0,
				null);
		canvas.restore();
	}
	...
	private void drawCarDot1(Canvas canvas) {
		canvas.save();
		canvas.translate(mCarDotX + mCarDot1XOffset
				+ mCarDriftedOffsetXDistance, mCarDotY + mCarDot1YOffset
				+ mCarDriftedOffsetYDistance);
	...
	private void drawCarDot2(Canvas canvas) {
		canvas.save();
		canvas.translate(mCarDotX + mCarDot2XOffset
				+ mCarDriftedOffsetXDistance, mCarDotY + mCarDot2YOffset
				+ mCarDriftedOffsetYDistance);
	...
	private void drawCarDot3(Canvas canvas) {
		canvas.save();
		canvas.translate(mCarDotX + mCarDot3XOffset
				+ mCarDriftedOffsetXDistance, mCarDotY + mCarDot3YOffset
				+ mCarDriftedOffsetYDistance);
	...
	private void drawCarDot4(Canvas canvas) {
		canvas.save();
		canvas.translate(mCarDotX + mCarDot4XOffset
				+ mCarDriftedOffsetXDistance, mCarDotY + mCarDot4YOffset
				+ mCarDriftedOffsetYDistance);
	...
	private void drawCarDot5(Canvas canvas) {
		canvas.save();
		canvas.translate(mCarDotX + mCarDot5XOffset
				+ mCarDriftedOffsetXDistance, mCarDotY + mCarDot5YOffset
				+ mCarDriftedOffsetYDistance);
	...
	private void drawCarOutFireFrame(Canvas canvas) {
		...
		canvas.save();
		canvas.translate(mCarOutFireX + mCarOutFireXOffset, mCarOutFireY
				+ mCarOutFireYOffset);
	...
	private void moveFrontWheelFire() {
		Log.d(TAG, "moveFrontWheelFire(),mFrontWheelFireIndex:"
				+ mFrontWheelFireIndex);
	...
	private void moveBehindWheelFire() {
		Log.d(TAG, "moveBehindWheelFire(),mBehindWheelFireIndex:"
				+ mBehindWheelFireIndex);
		if (mCarDriftedEndStepCount % 2 == 0) {
			return;
		}
	...
	private void moveLeftWheelSmog() {
		Log.d(TAG, "moveLeftWheelSmog(),mLeftWheelSmogIndex:"
				+ mLeftWheelSmogIndex);
		if (mCarDriftedEndStepCount % 2 == 0) {
			return;
		}
	...
	private void moveRightWheelSmog() {
		...
		Log.d(TAG, "moveRightWheelSmog(),mRightWheelSmogIndex:"
				+ mRightWheelSmogIndex);
	...
	private void destoryBitmap() {
		...
		if (mCarLightBmp != null) {
			mCarLightBmp.recycle();
			mCarLightBmp = null;
		}

		if (mCarInFireFrames != null) {
			for (int i = 0; i < 15; i++) {
				if (mCarInFireFrames[i] != null) {
					mCarInFireFrames[i].recycle();
					mCarInFireFrames[i] = null;
				}
			}
			mCarInFireFrames = null;
		}

		if (mFrontWheelFireFrames != null) {
			for (int i = 0; i < 5; i++) {
				if (mFrontWheelFireFrames[i] != null) {
					mFrontWheelFireFrames[i].recycle();
					mFrontWheelFireFrames[i] = null;
				}
			}
			mFrontWheelFireFrames = null;
		}

		if (mBehindWheelFireFrames != null) {
			for (int i = 0; i < 5; i++) {
				if (mBehindWheelFireFrames[i] != null) {
					mBehindWheelFireFrames[i].recycle();
					mBehindWheelFireFrames[i] = null;
				}
			}
			mBehindWheelFireFrames = null;
		}

		if (mLeftWheelSmogFrames != null) {
			for (int i = 0; i < 10; i++) {
				if (mLeftWheelSmogFrames[i] != null) {
					mLeftWheelSmogFrames[i].recycle();
					mLeftWheelSmogFrames[i] = null;
				}
			}
			mLeftWheelSmogFrames = null;
		}

		if (mRightWheelSmogFrames != null) {
			for (int i = 0; i < 10; i++) {
				if (mRightWheelSmogFrames[i] != null) {
					mRightWheelSmogFrames[i].recycle();
					mRightWheelSmogFrames[i] = null;
				}
			}
			mRightWheelSmogFrames = null;
		}
		...
		if (mCarOutFireFrames != null) {
			for (int i = 0; i < 4; i++) {
				if (mCarOutFireFrames[i] != null) {
					mCarOutFireFrames[i].recycle();
					mCarOutFireFrames[i] = null;
				}
			}
			mCarOutFireFrames = null;
		}
		System.gc();
	}
	...
	protected void drawCanvas() {
		...
		try {
			switch (state) {
			case STATE_CAR_MOVE_IN:
				drawCarAndLight(mCanvas);
				break;
			case STATE_CAR_MOVE_IN_AND_FIRE:
				drawCarInFireFrame(mCanvas);
				drawCarAndLight(mCanvas);
				break;
			case STATE_CAR_DRIFTED:
				drawCarAndLight(mCanvas);
			...
			case STATE_CAR_DRIFTED_END:
				if (mLeftWheelSmogIndex < 10) {
					drawLeftWheelSmog(mCanvas);
				}
				if (mRightWheelSmogIndex < 10) {
					drawRightWheelSmog(mCanvas);
				}
				drawCarAndLight(mCanvas);
				if (mFrontWheelFireIndex < 5) {
					drawFrontWheelFire(mCanvas);
				}
				if (mBehindWheelFireIndex < 5) {
					drawBehindWheelFire(mCanvas);
				}
				drawCarDots(mCanvas);
				break;
			case STATE_CAR_LIGHT_FLASH:
